...
public class ExecutionUtils implements Serializable {
   ...
   */
  public static MapReducer<OSMEntitySnapshot> snapshotFilter(MapReducer<OSMEntitySnapshot> mapRed, Set<OSMType> osmTypes1, Set<OSMType> osmTypes2, Set<SimpleFeatureType> simpleFeatureTypes1, Set<SimpleFeatureType> simpleFeatureTypes2, Integer[] keysInt1, Integer[] keysInt2, Integer[] valuesInt1, Integer[] valuesInt2) {
   ...
   */
  public static MapAggregator<OSHDBCombinedIndex<OSHDBTimestamp, Integer>, OSMEntitySnapshot> snapshotFilter(MapAggregator<OSHDBCombinedIndex<OSHDBTimestamp, Integer>, OSMEntitySnapshot> mapRed, Set<OSMType> osmTypes1, Set<OSMType> osmTypes2, Set<SimpleFeatureType> simpleFeatureTypes1, Set<SimpleFeatureType> simpleFeatureTypes2, Integer[] keysInt1, Integer[] keysInt2, Integer[] valuesInt1, Integer[] valuesInt2) {
    return mapRed.filter(snapshot -> snapshotMatches(snapshot, osmTypes1, simpleFeatureTypes1, keysInt1, valuesInt1) || snapshotMatches(snapshot, osmTypes2, simpleFeatureTypes2, keysInt2, valuesInt2));
  }
   ...
   */
  public static boolean snapshotMatches(OSMEntitySnapshot snapshot, Set<OSMType> osmTypes, Set<SimpleFeatureType> simpleFeatureTypes, Integer[] keysInt, Integer[] valuesInt) {
   ...
   */
  public static <A, U extends Comparable<U> & Serializable, V extends Comparable<V> & Serializable> SortedMap<V, SortedMap<U, A>> nest(Map<OSHDBCombinedIndex<U, V>, A> result) {
   ...
   */
  public void streamResponse(HttpServletResponse servletResponse, ExtractionResponse osmData, Stream<org.wololo.geojson.Feature> resultStream) throws Exception {
    ...
    ThreadLocal<ByteArrayOutputStream> outputBuffers = ThreadLocal.withInitial(ByteArrayOutputStream::new);
   ...
   */
  public void writeCsvResponse(GroupByObject[] resultSet, HttpServletResponse servletResponse, List<String[]> comments) {
    try {
   ...
   * requests.
   */
  public void writeCsvResponse(Result[] resultSet, HttpServletResponse servletResponse, List<String[]> comments) {
    try {
      servletResponse = setCsvSettingsInServletResponse(servletResponse);
      CSVWriter writer = writeComments(servletResponse, comments);
   ...
   */
  public static List<String[]> createCsvTopComments(String url, String text, String apiVersion, Metadata metadata) {
   ...
   */
  public org.wololo.geojson.Feature createOSMFeature(OSMEntity entity, Geometry geometry, Map<String, Object> properties, Set<Integer> keysInt, boolean includeTags, boolean includeOSMMetadata, boolean includeContributionTypes, boolean isContributionsEndpoint, ElementsGeometry elemGeom, EnumSet<ContributionType> contributionTypes) {
  ...
  public static <K extends Comparable<K> & Serializable, V extends Number> SortedMap<OSHDBCombinedIndex<OSHDBTimestamp, K>, V> computeResult(RequestResource requestResource, MapAggregator<OSHDBCombinedIndex<OSHDBTimestamp, K>, OSMEntitySnapshot> mapAgg) throws Exception {
    ...
    case PERIMETER: 
      return (SortedMap<OSHDBCombinedIndex<OSHDBTimestamp, K>, V>) mapAggGeom.sum(geom -> {
    ...
    case LENGTH: 
      return (SortedMap<OSHDBCombinedIndex<OSHDBTimestamp, K>, V>) mapAggGeom.sum(geom -> cacheInUserData(geom, () -> Geo.lengthOf(geom)));
    case AREA: 
      return (SortedMap<OSHDBCombinedIndex<OSHDBTimestamp, K>, V>) mapAggGeom.sum(geom -> cacheInUserData(geom, () -> Geo.areaOf(geom)));
    default: 
      return null;
    }
  }
   ...
   */
  @SuppressWarnings({"unchecked"})
  public static <K extends Comparable<K> & Serializable, V extends Number> SortedMap<OSHDBCombinedIndex<OSHDBCombinedIndex<Integer, K>, OSHDBTimestamp>, V> computeNestedResult(RequestResource requestResource, MapAggregator<OSHDBCombinedIndex<OSHDBCombinedIndex<Integer, K>, OSHDBTimestamp>, OSMEntitySnapshot> mapAgg) throws Exception {
    var mapAggGeom = mapAgg.map(OSMEntitySnapshot::getGeometry);
    switch (requestResource) {
    case COUNT: 
      return (SortedMap<OSHDBCombinedIndex<OSHDBCombinedIndex<Integer, K>, OSHDBTimestamp>, V>) mapAgg.count();
    case PERIMETER: 
      return (SortedMap<OSHDBCombinedIndex<OSHDBCombinedIndex<Integer, K>, OSHDBTimestamp>, V>) mapAggGeom.sum(geom -> {
        if (!(geom instanceof Polygonal)) {
          return 0.0;
        }
        return cacheInUserData(geom, () -> Geo.lengthOf(geom.getBoundary()));
    ...
    case LENGTH: 
      return (SortedMap<OSHDBCombinedIndex<OSHDBCombinedIndex<Integer, K>, OSHDBTimestamp>, V>) mapAggGeom.sum(geom -> cacheInUserData(geom, () -> Geo.lengthOf(geom)));
    case AREA: 
      return (SortedMap<OSHDBCombinedIndex<OSHDBCombinedIndex<Integer, K>, OSHDBTimestamp>, V>) mapAggGeom.sum(geom -> cacheInUserData(geom, () -> Geo.areaOf(geom)));
    default: 
      return null;
    }
  }
   ...
   */
  public static ElementsResult[] fillElementsResult(SortedMap<OSHDBTimestamp, ? extends Number> entryVal, boolean isDensity, DecimalFormat df, Geometry geom) {
   ...
   */
  public static ContributionsResult[] fillContributionsResult(SortedMap<OSHDBTimestamp, ? extends Number> entryVal, boolean isDensity, InputProcessor inputProcessor, DecimalFormat df, Geometry geom) {
   ...
   */
  public static Double[] fillElementsRatioGroupByBoundaryResultValues(Set<? extends Entry<OSHDBCombinedIndex<OSHDBTimestamp, Integer>, ? extends Number>> resultSet, DecimalFormat df) {
   ...
   */
  public static Pair<Pair<Integer, Integer>, OSMEntitySnapshot> mapSnapshotToTags(int keysInt, Integer[] valuesInt, OSMEntitySnapshot f) {
   ...
   */
  private static ImmutablePair<List<String>, List<String[]>> createCsvResponseForElementsGroupBy(GroupByObject[] resultSet) {
   ...
   * @return <code>Pair</code> containing the column names (left) and the data rows (right)
   */
  private static ImmutablePair<List<String>, List<String[]>> createCsvResponseForElementsRatioGroupBy(GroupByObject[] resultSet) {
    List<String> columnNames = new LinkedList<>();
    columnNames.add("timestamp");
    List<String[]> rows = new LinkedList<>();
    for (int i = 0; i < resultSet.length; i++) {
   ...
   * @return <code>Pair</code> containing the column names (left) and the data rows (right)
   */
  private static ImmutablePair<List<String>, List<String[]>> createCsvResponseForUsersGroupBy(GroupByObject[] resultSet) {
    List<String> columnNames = new LinkedList<>();
   ...
   */
  private void writeStreamResponse(ThreadLocal<JsonGenerator> outputJsonGen, Stream<org.wololo.geojson.Feature> stream, ThreadLocal<ByteArrayOutputStream> outputBuffers, final ServletOutputStream outputStream) throws ExecutionException, InterruptedException, IOException {
   ...
   */
  private static CSVWriter writeComments(HttpServletResponse servletResponse, List<String[]> comments) throws IOException {
   ...
   */
  private static Map<String, Object> addContributionTypes(Map<String, Object> properties, EnumSet<ContributionType> contributionTypes) {
